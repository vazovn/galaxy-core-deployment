...
Main:
{
    ...
    my (
        ...
        $end,         $executable, $help,        $jobId, $machine,
        $man,         $memory,     $name,        $nodes, $organization,
        $procs,       $project,    $qos,         $start, $type,
        $user,        $version,    %extensions
    );
    GetOptions(
    ...
    # Build request
    my $request = new Gold::Request(object => "Job", action => "Create");
    ...
    $request->setAssignment("JobId",      $jobId);
    ...
    # Obtain the job id just created
    my $job = $response->getDatumValue("Id");
    if ($response->getStatus() eq "Success")
    {
        $response->setMessage("Successfully created Job $job");
    }
...
gmkjob - create a new job record

=head1 SYNOPSIS

B<gmkjob> [B<-p> I<project_name>] [B<-u> I<user_name>] [B<-m> I<machine_name>] [B<-o> I<organization>] [B<-C> I<queue_name>] [B<-Q> I<quality_of_service>] [B<-P> I<processors>] [B<-N> I<nodes>] [B<-M> I<memory>] [B<-D> I<disk>] [B<-n> I<job_name>] [B<--application> I<application>] [B<--executable> I<executable>] [B<-t> I<wallclock_duration>] [B<-s> I<start_time>] [B<-e> I<end_time>] [B<-T> I<job_type>] [B<-d> I<description>] [B<-X | --extension> I<property>=I<value>]* [B<--debug>] [B<-?>, B<--help>] [B<--man>] [B<--quiet>] [B<-v>, B<--verbose>] [B<-V>, B<--version>] {[B<-J>] I<job_id>} 

=head1 DESCRIPTION

B<gmkjob> is used to create a new job record
